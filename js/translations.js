const translations = {
    ...
    uk: {
        ...
        aux_select_tooltip: "Канал AUX 3-х позиційного тоглу за допомогою якого буде змінюватися потужність відеопередавача. Зазвичай це тогл SC",
        ...
        vtx_serial_port_tooltip: "Номер порту UART до якого припаяно дріт керування VTX. Для SpeedyBee F405 це UART 1 (контактна площадка TX1).",
        vtx_protocol_tooltip: "Протокол керування який використовується у VTX. Визначається шаблоном, але може бути змінений вручну.",
        power_state_1_tooltip: "Значення потужності для першого положення тоглу (вгору). Зазвичай це найнижче (25мВт) значення потужністі. Фактично це порядковий номер значення потужності з таблиці частот. Нижче ви можете змінити ліміти для визначення цього положення тоглу.",
        power_state_2_tooltip: "Значення потужності для другого положення тоглу (по центру). Зазвичай це середнє значення потужністі. Фактично це порядковий номер значення потужності з таблиці частот. Нижче ви можете змінити ліміти для визначення цього положення тоглу.",
        power_state_3_tooltip: "Значення потужності для третього положення тоглу (вниз). Зазвичай це максимальне значення потужністі. Фактично це порядковий номер значення потужності з таблиці частот. Нижче ви можете змінити ліміти для визначення цього положення тоглу.",
        ...
        band_select_tooltip: "Канал AUX 3-х позиційного тоглу за допомогою якого буде змінюватися діапазон. Не можна обрати AUX який вже задіяний для керування потужністю VTX. Не має стандартного значення, обовʼязково треба обрати вручну для продовження роботи.",
        band_state_1_tooltip: "Діапазон який буде активовано для першого положення тоглу (вгору). Нижче ви можете змінити ліміти для визначення цього положення. Увага‼️ діапазони 6-8 можуть не підтримуватися вашим VTX. Обирайте тільки, якщо ви впевнені, що вони є.",
        band_state_2_tooltip: "Діапазон який буде активовано для другого положення тоглу (по центру). Нижче ви можете змінити ліміти для визначення цього положення. Увага‼️ діапазони 6-8 можуть не підтримуватися вашим VTX. Обирайте тільки, якщо ви впевнені, що вони є.",
        band_state_3_tooltip: "Діапазон який буде активовано для для третього положення тоглу (вниз). Нижче ви можете змінити ліміти для визначення цього положення. Увага‼️ діапазони 6-8 можуть не підтримуватися вашим VTX. Обирайте тільки, якщо ви впевнені, що вони є.",
    }
};
